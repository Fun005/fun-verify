<template>
  <div class="verify3-container">
    <!-- 验证码容器 -->
    <components v-if="componentType" :is="componentType" :verifyType="verifyType" :figure="figure" :arith="arith" :width="width" :height="height" :fontSize="fontSize" :codeLength="codeLength" :mode="mode" :vOffset="vOffset" :vSpace="vSpace" :explain="explain" :imgUrl="imgUrl" :imgName="imgName" :showButton="showButton" :imgSize="imgSize" :blockSize="blockSize" :barSize="barSize" :defaultNum="defaultNum" :checkNum="checkNum" ref="instance"></components>
    <!-- 确定按钮容器 -->
    <div @click="checkCode" v-show="showButton" style="width:0; height:0;">
      <slot name="check">
        <button class="verify-btn">ok</button>
      </slot>
    </div>
  </div>
</template>
